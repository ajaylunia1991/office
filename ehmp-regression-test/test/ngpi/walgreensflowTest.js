Feature('NGPI - Walgreens sanity testing');

var config = require('config');
var domain = config.get('EHM');
var ngpiTest = require(process.cwd() + '/data/ngpiTest.js')();
if( process.env.NODE_ENV == 'cm') {
  dataEnv = Data(ngpiTest.ngpidataCMWG); // cm data
}
if( process.env.NODE_ENV == 'qa') {
  dataEnv = Data(ngpiTest.ngpidataQAWG); // qa data
}


dataEnv.Scenario('NGPI Walgreens flow', { retries: 1 }, async (I, current, ngpiPage) => {
