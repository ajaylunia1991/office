Feature('NGPI - Summit Medical Group  sanity testing');

var config = require('config');
var domain = config.get('EHM');
var ngpiTest = require(process.cwd() + '/data/ngpiTest.js')();
if( process.env.NODE_ENV == 'cm') {
  dataEnv = Data(ngpiTest.ngpidataCMSM); // cm data
}
if( process.env.NODE_ENV == 'qa') {
  dataEnv = Data(ngpiTest.ngpidataQASM); // qa data
}


dataEnv.Scenario('NGPI SMG flow', { retries: 0 }, async (I, current, ngpiPage) => {
