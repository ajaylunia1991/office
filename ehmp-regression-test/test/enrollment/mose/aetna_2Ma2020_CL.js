...
Data(planDetails.aetna_2).Scenario('MA_aetna_DC_Cassie', async(I, current, mosePage) =>{
    if( process.env.NODE_ENV == 'prod' ||  process.env.NODE_ENV == 'gpa' ){
        ...
        // I.see('Aetna Medicare Premier Plan (PPO)');
        I.executeScript(simpleCase.autoRun);I.wait(5);

        //filled out Eligibility
        I.seeInCurrentUrl('/Eligibility');
        // I.seeInSource('Primary Care Dentist Full Name');
        // I.seeInSource('Aetna Dental Office ID');
        I.executeScript(simpleCase.autoRun);I.wait(5);
