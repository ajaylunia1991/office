...
Scenario('Sagarika - addDrugs -should check tracking for drugs of MDC', {retries: 0}, async(I, recommendation_homePage) => {
    ...
    if( process.env.NODE_ENV == 'prod' ||  process.env.NODE_ENV == 'gpa'){
        ...
        I.wait(3);
        I.amOnPage(domain +'/recommendation/#/drug_savings');
        //waitimg for the plugin to load
        I.waitForVisible('#loadComplete',30);//wait for plugin
        I.waitForText('Estimated drug cost with Medicare Advantage plan with the lowest drug cost', 40);
        I.wait(3);

       
        //get report name by time
        let time = await moment().tz('America/Los_Angeles').format('YYYY-MM-DD-HH');
        let filePath = 'file:///C://Users//trackUser//Downloads//medicare.adddrugs.' + time +'.html'
        ...
        //adding assertion 
        I.see('pageName: "MedicareCom:Recommendation:findPlan:DrugSavings"');
