Feature('mdc select medicare screen tracking test');
...
Scenario('Recommendation Homepage UI Test@RecoPartnerFlowToEHMP', async function (I, homePage) {
  ...
  //  I.click('//*[@id="menu-item-356896"]/a'); //click on Recommandation
    ...
    homePage.clickIamNewToMedicare();
    // console.log("Whoo..Whoo...Reco Home Page is up and running");
homePage.clickZipCode();
homePage.clickLocationOverview();
homePage.clickProdLineIntroQuestions();
homePage.clickAddDrugs();
homePage.clickAddDoctors();
...
// **************if it's on PROD, verify the logo is showing up
    ...
    if (process.env.NODE_CONFIG_ENV == 'pr' ) {
        I.wait(1);
...
//    }
  ...
  //click on enroll btn on MDC RECO last page to get into EHMP application
    ...
    I.fillField(homePage.fields.reportFN,randomInfo.firstName);
    I.fillField(homePage.fields.reportLN, randomInfo.lastName);
    I.fillField(homePage.fields.reportEMAIL, randomInfo.email);
    if (process.env.NODE_CONFIG_ENV == 'cm' | process.env.NODE_CONFIG_ENV == 'qa' ) {

    I.click(homePage.fields.reportBtn);
    await I.wait(8);
