Feature('NGPI - Summit Bose Sanity testing---created by Aneesha');

var config = require('config');
var domain = config.get('NGPIBOURL');
var ngpiBose = require(process.cwd() + '/data/ngpiBose.js')();
if( process.env.NODE_ENV == 'cm') {
  dataEnv = Data(ngpiBose.ngpiBoseCMSM); // cm data
}
if( process.env.NODE_ENV == 'qa') {
  dataEnv = Data(ngpiBose.ngpiBoseQASM); // qa data
}


dataEnv.Scenario('NGPI SMG flow', { retries: 0 }, async (I,current, ngpiBosePage) => {
  console.log('Testing the Bose Page for the NGPI flow');
  var num = Math.floor(Math.random()*9000+1000);
 // var aniPhoneNm = '464766'+num;
 var aniPhoneNm =4646768789;
  ...
  ngpiBosePage.selectPage3Details();
